...
cmd:option('-mixture', 0, [[With hard attention, train as mixture instead of argmax]])
...
if opt.mixture == 1 then
  function nn.ReinforceCategorical:updateOutput(input)
   ...
   while (not done) and (i < n) do
      ...
      -- All the scores available.
       ...
       if i <= source_l + 1 then
         ...
         total_attn_counts = total_attn_counts + 1
       end
       --print('correct:', correct_attn_counts)
       --print('total:', total_attn_counts)
...
function main()
   ...
   -- counts for attention accuracy
   total_attn_counts = 0
   correct_attn_counts = 0 
   for line in file:lines() do
      sent_id = sent_id + 1
   ...
   print('attention accuracy:', correct_attn_counts / total_attn_counts)
